...
let parseNote (note: string): Note =
    ...
    { Title = lines.[0].[7..]
      TagList = lines.[1].Split "; " |> Array.toList
      Notebook = lines.[2].[10..]
...
let advancedSearch (args: string list): string =
    let parseTerm (term: string): SearchTerm option =
        ...
        let baseTerm: SearchTerm option =
            ...
            | [ "ti"; word ]
            | [ "ti"; word; _ ] ->
                Some({ Body = Title(word); Flag = None })
            | [ "ta"; words ]
            | [ "ta"; words; _ ] ->
                Some({ Body = TagList(words.Split(",") |> Array.toList); Flag = None })
            | [ "nb"; word ]
            | [ "nb"; word; _ ] ->
                Some({ Body = Notebook(word); Flag = None })
            | [ "cr"; date ]
            | [ "cr"; date; _ ] ->
        ...
        match baseTerm with
        | Some(baseT) & (Some({ Body = Title(_); Flag = _ }) | Some({ Body = TagList(_); Flag = _ }) | Some({ Body = Notebook(_); Flag = _ })) ->
            match terms with
            | [ _; _ ]
            | [ _; _; "iW" ]
            | [ _; _; "Wi" ]
            | [ _; _; "i" ]
            | [ _; _; "W" ] ->
                Some({ baseT with Flag = Some(TextFlag(ignoreCase = true,
                                                     wholeWord = false)) })
            | [ _; _; "iw" ]
            | [ _; _; "wi" ]
            | [ _; _; "w" ] ->
            ...
            | [ _; _; "Iw" ]
            | [ _; _; "wI" ] ->
                Some({ baseT with Flag = Some(TextFlag(ignoreCase = false,
                                                     wholeWord = true)) })
            | [ _; _; "IW" ]
            | [ _; _; "WI" ]
            | [ _; _; "I" ] ->
                Some({ baseT with Flag = Some(TextFlag(ignoreCase = false,
                                                     wholeWord = false)) })
            | _ -> None
        | Some(baseT) & (Some({ Body = Created(_); Flag = _ }) | Some( { Body = Updated(_); Flag = _ })) ->
            match terms with
            | [ _; _; "b" ] ->
                Some({ baseT with Flag = Some(DateFlag(beforeDate = true)) })
            | [ _; _; "B" ] ->
                Some({ baseT with Flag = Some(DateFlag(beforeDate = false)) })
            | [ _; _; flag ] ->
